...
export default function MedicinesView({ medicines, categories = [], loading, error }) {
  ...
  const resolveCategoryName = (medicine) => {
    ...
    const rawCategoryValue = medicine.category_id ?? medicine.categoryId ?? medicine.category?.id ?? medicine.category?._id ?? medicine.category?.name ?? medicine.category;
  ...
  const filteredMedicines = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter((medicine) => {
        ...
        const category = resolveCategoryName(medicine).toLowerCase();
        const price = String(Number(medicine.price || 0).toFixed(2));
        const expiryDate = medicine.expiryDate ? new Date(medicine.expiryDate).toLocaleDateString().toLowerCase() : '';
        return name.includes(normalizedSearch) || category.includes(normalizedSearch) || price.includes(normalizedSearch) || expiryDate.includes(normalizedSearch);
      });
    }
  ...
  return (
    <main className="admin-manage-page">
      ...
      <div className="manage-medicines-container">
        <div className="medicine-header staff-medicine-header">
          <h1>Medicine List</h1>
          <p>View all medicines and their current details</p>
        </div>
        ...
        <div className="medicine-list">
          <div className="list-header">
            ...
            <span className="med-expiry">Expiry Date</span>
          ...
          <div className="list-body">
            {filteredMedicines.length > 0 ? (
              filteredMedicines.map((medicine) => (
                <div className="medicine-item" key={medicine._id}>
                  <span className="med-name">{medicine.name}</span>
                  ...
                  <span className="med-expiry">{formatExpiryDate(medicine.expiryDate)}</span>
